...
public class Dice{
	...
	private int dieFace;
	int currentScore;
	private ArrayBag<Integer> dieBag;
	private ArrayBag<Integer> freqStack;
	private ArrayBag<String> availableCategories;
	private int freqOne;
	private int freqTwo;
	...
	public Dice() {
		dieBag= new ArrayBag<>(5);
		freqStack= new ArrayBag<>(6);
		currentScore=0;
		availableCategories= new ArrayBag<>(13);
		freqOne= freqTwo=freqThree=freqFour=freqFive=freqSix=0;

	}
	public ArrayBag<Integer> rollDie(){
dieBag.clear();
	...
	public ArrayBag<Integer> getFrequencies() {
	...
	public ArrayBag<String> getCategories(){
	...
	public int getRoundScore(String selection) {
		//upper Section
		if(selection=="1")
			currentScore=1*dieBag.getFrequencyOf(1);
		else if(selection=="2")
			currentScore=2*dieBag.getFrequencyOf(2);
		else if(selection=="3")
			currentScore=3*dieBag.getFrequencyOf(3);
		else if(selection=="4")
			currentScore=3*dieBag.getFrequencyOf(4);
		else if(selection=="5")
			currentScore=3*dieBag.getFrequencyOf(5);
		else if(selection=="6")
			currentScore=3*dieBag.getFrequencyOf(6);

		//lower Section
		else if(selection=="7"|| selection=="8") {
			while(!dieBag.isEmpty()) {
				currentScore=+dieBag.remove();
			}
		}
		else if (selection=="9")
			currentScore=25;
